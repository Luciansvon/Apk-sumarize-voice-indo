...
if [ -f "$LIBS_DIR/$SHERPA_AAR" ]; then
    ...
    echo "Downloading sherpa-onnx v${SHERPA_VERSION} AAR (~54MB)..."
    curl -fL --retry 4 --retry-delay 2 -o "$LIBS_DIR/$SHERPA_AAR" "$SHERPA_URL"
...
echo "on first app launch (~604MB total, requires internet once)."
